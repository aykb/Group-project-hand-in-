...
1.  **Load the Daily Energy Dataset**: The `energy_daily_features.csv` dataset (generated in Challenge 1) is loaded and inspected.
...
3.  **Train/Test Split**: The data is split into training (80%) and testing (20%) sets. Features include calendar and lag features, as well as one-hot encoded `building_type` and `location`.
